<template>
  <div class="graph-content-wrap">
    ...
    >
      ...
      <path id="zoom_area" style="fill: #4E73FF; stroke: #606BE1; stroke-width:1px; opacity:0.3" />
    </svg>
...
export default class GraphContent extends Vue {
  ...
  private async handleMouseMove(event: MouseEvent) {
    ...
    if (this.isMouseDownFiredByNode) {
      ...
      if (this.box.boxing) {
        ...
        const path = `M${this.box.startX},${this.box.startY}H${this.box.endX}V${this.box.endY}H${this.box.startX}Z`
        this.rect.setAttribute('d', path)
        this.checkSelected()
      }
  ...
  private checkNodeRange(item: Workflow.WorkflowNodeVo) {
    if (
      ...
      this.range.startY / this.scale - this.translateY
    ) {
      return false
    } else if (item.posY > this.range.endY / this.scale - this.translateY) {
      return false
    } else if (
      item.posX + this.nodeWidth <
      this.range.startX / this.scale - this.translateX
    ) {
      return false
    } else if (item.posX > this.range.endX / this.scale - this.translateX) {
      return false
    } else {
      return true
    }
