...
interface ItemClass<T> {
  new (model: { id?: string; [key: string]: any }, ...agrs: any[]): T
}
...
class RecordCollection<T extends object> {
  private readonly _records: Record<string, T> = {}

  readonly itemName: string
  readonly itemClass: ItemClass<T>

  constructor(itemClass: ItemClass<T>) {
    this.itemName = itemClass.name
  ...
  addItem(...args: ConstructorParameters<ItemClass<T>>): T | undefined {
    if (args[0].id && args[0].id in this._records) {
      console.warn(
        `can't add ${this.itemName}, exist node where id is '${args[0].id}'`
      )
      return
    }
    const item = new this.itemClass(...args)
    const id = 'id' in item ? (item as any).id : uniqueId(this.itemName)
    this._records[id] = item
    return item
  }
  ...
  clear() {
    Object.keys(this._records).forEach(id => {
      this.deleteItem(id)
    })
  }
}
...
export class Store {
  graph: Graph
  readonly nodeCollection = new RecordCollection(Node)
  ...
  public findNode(id: string | number): INode | undefined {
    return this.nodeCollection.findItem(`${id}`)
  }
  ...
  public deleteNode(id: string): INode | undefined {
    ...
    this.nodeCollection.deleteItem(id)
  ...
  public addNode(item: INodeModel): INode | undefined {
    ...
    const nodeCfg = Object.assign(
      defaultNodeCfg,
      { direction },
      this.graph.get('nodeInfo') || {}
    )
    const node = this.nodeCollection.addItem(model, nodeCfg)
    if (!node) {
      return
    }
  ...
  public sortByZIndex() {
    const nodes = this.nodeCollection.items
