    ...
    <MtdTable :data="scalingGroups">
      ...
      <MtdTableColumn label="状态" v-if="isShowStatus" width="100px">
        <template slot-scope="scope">
          <span :class="tfStatusMapping(scope.row.status).class">{{
            tfStatusMapping(scope.row.status).text
          }}</span>
        </template>
      </MtdTableColumn>
...
import TFScaleGroupModal from '@/components/service/customed/TFScaleGroupModal.vue'
