    ...
    >
      <template slot="columns">
        <MtdTableColumn label="任务名" prop="label">
          <template slot-scope="scope">
            <router-link
              ...
              :to="jobViewDetailLink({ id, templateId, jobId: scope.row.id })"
        ...
        <MtdTableColumn label="伸缩组" show-overflow-tooltip>
          <template slot-scope="scope">
            <span v-for="scale in scope.row.scalingGroups" :key="scale.id">{{
              scale.name + ' '
            }}</span>
          </template>
        </MtdTableColumn>
        <MtdTableColumn label="状态" width="100">
          <template slot-scope="scope">
            <span :class="`${statusMapping(scope.row.status).class}`">{{
              statusMapping(scope.row.status).text
            }}</span>
          </template>
        </MtdTableColumn>
...
import { jobViewDetailLink } from '@/metaData/customed'
...
export default class CustomedTemplateJobs extends Vue {
  @Prop({
    type: String,
    required: true
  })
