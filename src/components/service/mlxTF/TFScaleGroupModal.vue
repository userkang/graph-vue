<template>
  <CommonModal :title="`${handleType === 'add' ? '添加' : '编辑'}伸缩组`" @close="handleClose">
    <MtdForm :model="submitForm" :label-width="120">
      ...
      <MtdFormItem label="运行队列" prop="queue">
        <MtdSelect v-model="submitForm.queue" class="scale-form-item">
          <div>
            <MtdOption
      ...
      <MtdFormItem label="显存类型" prop="queue">
        <MtdSelect v-model="submitForm.gcores" class="scale-form-item">
          <div>
            <MtdOption v-for="item in gcroes" :key="item" :label="item" :value="item" />
          </div>
        </MtdSelect>
        <ToolTip style="margin-left: 7px">若显存列表为空，就不用选择</ToolTip>
      </MtdFormItem>
...
import { MLXTFScalingQueuesStore } from '@/stores/service/MLXTF'
...
export default class TFScaleGroupModal extends Vue {
  ...
  private async mounted() {
    ...
    if (this.submitForm.hdfsCluster) {
      await MLXTFScalingQueuesStore.request(this.submitForm.hdfsCluster)
    }
  }
